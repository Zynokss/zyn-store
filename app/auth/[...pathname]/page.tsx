...
interface NeonAuthAppearance {
  baseTheme: string;
  layout: { variant: string; split: boolean };
  variables: Record<string, string>;
  darkModeVariables: Record<string, string>;
  elements: Record<string, string>;
}

interface NeonAuthViewProps {
  pathname: string;
  appearance: NeonAuthAppearance;
  fallbackPath: string;
...
export default function AuthCatchAllPage() {
  ...
  const authViewProps: NeonAuthViewProps = {
    pathname: `/auth/${tail}`,
    appearance: {
      baseTheme: 'zinc',
      ...
      <div className="flex-1 flex items-center justify-center p-4 my-8 sm:my-12">
        <div className="w-full max-w-2xl neon-auth-root">
          ...
          <div
            ...
            style={{ '--better-radius': '2px' } as React.CSSProperties}
          >
            <AuthView {...authViewProps} />
          </div>
